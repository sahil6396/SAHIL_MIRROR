...
async def _mirror_leech(client, message, isQbit=False, isLeech=False, sameDir=None, bulk=[]):
    ...
    # --- VIDTOOLS SELECTION (Fixed Dummy Class) ---
    vid_mode = None
    if vidTool:
        LOGGER.info(f"VidTools Triggered for user {message.from_user.id}") # Debug Log
        ...
        class Dummy:
            def __init__(self, msg):
                ...
                self.client = client
                self.tag = msg.from_user.mention
                self.uid = msg.id
                # These are needed by some selector logic
                self.isSuperGroup = msg.chat.type.name in ['SUPERGROUP', 'CHANNEL'] 
                self.excep_chat = False 

        vid_mode = await SelectMode(Dummy(message)).get_buttons()
        
        if not vid_mode:
            LOGGER.info("VidTools Cancelled or Timed Out")
            await delete_links(message)
            return
        
        LOGGER.info(f"VidMode Selected: {vid_mode}")
    # -----------------------------------------------
    ...
    if not isLeech:
        ...
        if user_dump and (user_dump.isdigit() or user_dump.startswith('-')):
            ...
            if not up and len(ldumps) == 1:
                ...
                u_data = user_data.get(message.from_user.id, {})
                if u_data.get('dump_all', False):
                    sel = u_data.get('dump_selection', [])
                    if sel:
                        up = [id_ for name, id_ in ldumps.items() if name in sel]
                    else:
                        up = list(ldumps.values())
                else:
