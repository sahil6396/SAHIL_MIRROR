...
GENERAL_METADATA = environ.get('GENERAL_METADATA', '')
if len(GENERAL_METADATA) == 0:
    GENERAL_METADATA = ''

VIDEO_METADATA = environ.get('VIDEO_METADATA', '')
if len(VIDEO_METADATA) == 0:
    VIDEO_METADATA = ''

AUDIO_METADATA = environ.get('AUDIO_METADATA', '')
if len(AUDIO_METADATA) == 0:
    AUDIO_METADATA = ''

SUBTITLE_METADATA = environ.get('SUBTITLE_METADATA', '')
if len(SUBTITLE_METADATA) == 0:
    SUBTITLE_METADATA = ''
...
ATTACHMENT_NAME = environ.get('ATTACHMENT_NAME', '')
if len(ATTACHMENT_NAME) == 0:
    ATTACHMENT_NAME = ''

AUDIOCHANGE = environ.get('AUDIOCHANGE', '')
if len(AUDIOCHANGE) == 0:
    AUDIOCHANGE = ''

AUDIOFILE = environ.get('AUDIOFILE', '')
if len(AUDIOFILE) == 0:
    AUDIOFILE = ''

AUDIOREMOVE = environ.get('AUDIOREMOVE', '')
if len(AUDIOREMOVE) == 0:
    AUDIOREMOVE = ''

AUDIOBITRATE = environ.get('AUDIOBITRATE', '')
if len(AUDIOBITRATE) == 0:
    AUDIOBITRATE = ''

AUDIOLANGUAGE = environ.get('AUDIOLANGUAGE', '')
if len(AUDIOLANGUAGE) == 0:
    AUDIOLANGUAGE = ''
...
INTROSUB = environ.get('INTROSUB', '')
if len(INTROSUB) == 0:
    INTROSUB = ''

REMSUB = environ.get('REMSUB', '')
if len(REMSUB) == 0:
    REMSUB = ''

WATERMARK = environ.get(' WATERMARK', '')
if len( WATERMARK) == 0:
     WATERMARK = ''
...
REPLACEMENT_INPUT = environ.get('REPLACEMENT_INPUT', '')
if len(REPLACEMENT_INPUT) == 0:
     REPLACEMENT_INPUT = ''
...
LEECH_CAPTION_REPLACE = environ.get('LEECH_CAPTION_REPLACE', '')
if len(LEECH_CAPTION_REPLACE) == 0:
    LEECH_CAPTION_REPLACE = ''
...
MIRROR_FILENAME_AUTORENAME = environ.get('MIRROR_FILENAME_AUTORENAME', '')
if len(MIRROR_FILENAME_AUTORENAME) == 0:
    MIRROR_FILENAME_AUTORENAME = ''
...
WELCOME_IMAGES = environ.get('WELCOME_IMAGES', 'https://i.ibb.co/5hxsGzkK/photo-2025-06-16-23-53-05-7516699552666091536.jpg')
WELCOME_IMAGES = (WELCOME_IMAGES.replace("'", '').replace('"', '').replace(
    '[', '').replace(']', '').replace(",", "")).split()

GOODBYE_IMAGES = environ.get('GOODBYE_IMAGES', 'https://i.ibb.co/tS1H6T2/photo-2025-06-16-23-53-05-7516699599910731796.jpg')
GOODBYE_IMAGES = (GOODBYE_IMAGES.replace("'", '').replace('"', '').replace(
    '[', '').replace(']', '').replace(",", "")).split()
...
config_dict = {'ANIME_TEMPLATE': ANIME_TEMPLATE,
               ...
               'LEECH_CAPTION_REPLACE': LEECH_CAPTION_REPLACE,
               ...
               'MIRROR_FILENAME_AUTORENAME': MIRROR_FILENAME_AUTORENAME,
...
def restart_qbclient():
    global qb_client
    try:
        qb_client.app_version()
    except Exception as e:
        log_error(f"[QB] Reconnecting qBittorrent client due to error: {e}")
        try:
            qb_client = get_client()
            log_info("[QB] qBittorrent client reconnected successfully.")
        except Exception as ex:
            log_error(f"[QB] Failed to reconnect qBittorrent: {ex}")
            
scheduler.add_job(restart_qbclient,IntervalTrigger(minutes=10),id="restart_qbclient",replace_existing=True)
