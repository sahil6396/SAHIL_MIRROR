...
class MirrorStatus:
    ...
    STATUS_AUDIOREMOVE   = "Audio Removing"
    STATUS_AUDIOCHANGE   = "Audio Changeing"
    STATUS_AUDIOLANGUAGE = "Changing Audio Language"
    STATUS_AUDIOBITRATE  = "Editing Audio Bitrate"
    STATUS_SUBTITLE      = "Adding Subtitles"
    STATUS_AUDIOFILE     = "Muxing Audio Track"
...
def arg_parser(items, arg_base):
    ...
    while i + 1 <= t:
        ...
        if part in arg_base:
            if arg_start == -1:
                arg_start = i
            if i + 1 == t and part in bool_arg_set or part in ['-s', '-j', '-vt']:
                arg_base[part] = True
            else:
                sub_list = []
