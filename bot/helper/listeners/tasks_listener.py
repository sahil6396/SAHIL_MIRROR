...
from bot.helper.ext_utils.media_utils import edit_metadata, add_subtitles, edit_audioremove, edit_audiochange, \
    edit_audiolanguage, add_audiofile, change_audiobitrate, remove_subtitles, cut_video_segments, parse_cut_segments, edit_audio_sort
from bot.helper.ext_utils.leech_utils import split_file, format_filename, get_document_type, get_tmdb_lookup
...
from bot.helper.mirror_utils.status_utils.subtitle_status import SubtitleStatus
from bot.helper.mirror_utils.status_utils.introsub_status import IntroSubStatus
from bot.helper.mirror_utils.status_utils.audiolanguage_status import AudioLanguageStatus
...
from bot.helper.mirror_utils.status_utils.audiofile_status import AudioFileStatus
from bot.helper.mirror_utils.status_utils.rclone_status import RcloneStatus
from bot.helper.mirror_utils.status_utils.audiobitrate_status import AudioBitrateStatus
...
class MirrorLeechListener:
    ...
    async def onDownloadComplete(self):
        ...
        if audiofile := self.user_dict.get('audiofile') or config_dict['AUDIOFILE']:
            smart_sync = self.user_dict.get('smart_sync', False) # <--- GET SETTING
            meta_path = up_path
            self.newDir = f'{self.dir}10000'
            await makedirs(self.newDir, exist_ok=True)
            async with download_dict_lock:
                download_dict[self.uid] = AudioFileStatus(name, size, gid, self)
            if await aiopath.isfile(meta_path) and (await get_document_type(meta_path))[0]:
                ...
                # PASS SETTING HERE 👇
                await add_audiofile(self, base_dir, meta_path, outfile, audiofile, smart_sync)
                if self.suproc == 'cancelled':
                    return
                ...
                for dirpath, _, files in await sync_to_async(walk, meta_path):
                    for file in files:
                        ...
                        if (await get_document_type(video_file))[0]:
                            ...
                            # PASS SETTING HERE 👇
                            await add_audiofile(self, dirpath, video_file, outfile, audiofile, smart_sync)

        if audioremove := self.user_dict.get('audioremove') or config_dict['AUDIOREMOVE']:
            meta_path = up_path
            self.newDir = f'{self.dir}10000'
            await makedirs(self.newDir, exist_ok=True)
            async with download_dict_lock:
        ...
        if audiochange := self.user_dict.get('audiochange') or config_dict['AUDIOCHANGE']:
            meta_path = up_path
            self.newDir = f'{self.dir}10000'
            await makedirs(self.newDir, exist_ok=True)
            async with download_dict_lock:
        ...
        # --- ADD THIS BLOCK ---
        if audio_sort := self.user_dict.get('audio_sort'):
            meta_path = up_path
            self.newDir = f'{self.dir}10000'
            await makedirs(self.newDir, exist_ok=True)
            async with download_dict_lock:
                # We can reuse AudioChangeStatus or AudioLanguageStatus for the progress bar
                download_dict[self.uid] = AudioChangeStatus(name, size, gid, self)
            
            if await aiopath.isfile(meta_path) and (await get_document_type(meta_path))[0]:
                base_dir, file_name = ospath.split(meta_path)
                outfile = ospath.join(self.newDir, file_name)
                await edit_audio_sort(self, base_dir, meta_path, outfile, audio_sort)
                if self.suproc == 'cancelled':
                    return
            elif await aiopath.isdir(meta_path):
                for dirpath, _, files in await sync_to_async(walk, meta_path):
                    for file in files:
                        ...
                        if (await get_document_type(video_file))[0]:
                            outfile = ospath.join(self.newDir, file)
                            await edit_audio_sort(self, dirpath, video_file, outfile, audio_sort)
        # ----------------------

        if audiobitrate := self.user_dict.get('audiobitrate') or config_dict['AUDIOBITRATE']:
            meta_path = up_path
            self.newDir = f'{self.dir}10000'
            await makedirs(self.newDir, exist_ok=True)
            async with download_dict_lock:
                download_dict[self.uid] = AudioBitrateStatus(name, size, gid, self)
            if await aiopath.isfile(meta_path) and (await get_document_type(meta_path))[0]:
                base_dir, file_name = ospath.split(meta_path)
                outfile = ospath.join(self.newDir, file_name)
                await change_audiobitrate(self, base_dir, meta_path, outfile, audiobitrate)
                if self.suproc == 'cancelled':
                    return
            elif await aiopath.isdir(meta_path):
                for dirpath, _, files in await sync_to_async(walk, meta_path):
                    for file in files:
                        ...
                        if (await get_document_type(video_file))[0]:
                            outfile = ospath.join(self.newDir, file)
                            await change_audiobitrate(self, dirpath, video_file, outfile, audiobitrate)

        # --- VIDTOOLS PROCESSING ---
        if self.vidMode:
            # FIX: If extraction didn't run, up_path is empty. Set it to dl_path.
            if not up_path:
                up_path = dl_path
                
            LOGGER.info(f"Starting VidTools with path: {up_path}")
            ...
            LOGGER.info(f"VidTools finished. New path: {up_path}")
        # --------------------------------------------------------
        ...
        if self.compress:
            ...
            elif self.user_dict.get('merge_vid'):
        ...
        # --- MEDIA PROCESSING (Audio, Trim, etc) ---     

        if remsub := self.user_dict.get('remsub') or config_dict['REMSUB']:
            meta_path = up_path
            self.newDir = f'{self.dir}10000'
            await makedirs(self.newDir, exist_ok=True)
            async with download_dict_lock:
            ...
            if await aiopath.isfile(meta_path) and (await get_document_type(meta_path))[0]:
                base_dir, file_name = ospath.split(meta_path)
                outfile = ospath.join(self.newDir, file_name)
                await remove_subtitles(self, base_dir, meta_path, outfile, remsub)
                if self.suproc == 'cancelled':
                    return
            elif await aiopath.isdir(meta_path):
                for dirpath, _, files in await sync_to_async(walk, meta_path):
                    for file in files:
                        ...
                        if (await get_document_type(video_file))[0]:
                            outfile = ospath.join(self.newDir, file)
                            await remove_subtitles(self, dirpath, video_file, outfile, remsub)

        if subtitle  := self.user_dict.get('subtitle') or config_dict['SUBTITLE']:
            meta_path = up_path
            self.newDir = f'{self.dir}10000'
            await makedirs(self.newDir, exist_ok=True)
            async with download_dict_lock:
                download_dict[self.uid] = SubtitleStatus(name, size, gid, self)
            if await aiopath.isfile(meta_path) and (await get_document_type(meta_path))[0]:
                base_dir, file_name = ospath.split(meta_path)
                outfile = ospath.join(self.newDir, file_name)
                await add_subtitles(self, base_dir, meta_path, outfile, subtitle)
                if self.suproc == 'cancelled':
                    return
            elif await aiopath.isdir(meta_path):
                for dirpath, _, files in await sync_to_async(walk, meta_path):
                    for file in files:
                        ...
                        if (await get_document_type(video_file))[0]:
                            outfile = ospath.join(self.newDir, file)
                            await add_subtitles(self, dirpath, video_file, outfile, subtitle)
            ...
            if await aiopath.isfile(meta_path) and (await get_document_type(meta_path))[0]:
                ...
                for dirpath, _, files in await sync_to_async(walk, meta_path):
                    for file in files:
                        ...
                        if (await get_document_type(video_file))[0]:
                            outfile = ospath.join(self.newDir, file)
                            await edit_audiolanguage(self, dirpath, video_file, outfile, audiolanguage)

        if cut_input := self.user_dict.get('cut') or config_dict.get('CUT'):
            meta_path = up_path
            self.newDir = f'{self.dir}10000'
            await makedirs(self.newDir, exist_ok=True)
            async with download_dict_lock:
                download_dict[self.uid] = MetadataStatus(name, size, gid, self)
            segments = parse_cut_segments(cut_input)
            if not segments:
                LOGGER.warning(f"❌ Invalid cut segments provided: {cut_input}")
                return
            if await aiopath.isfile(meta_path) and (await get_document_type(meta_path))[0]:
                base_dir, file_name = ospath.split(meta_path)
                outfile = ospath.join(self.newDir, file_name)
                await cut_video_segments(self, base_dir, meta_path, outfile, segments)
                if self.suproc == 'cancelled':
                    return
            elif await aiopath.isdir(meta_path):
                for dirpath, _, files in await sync_to_async(walk, meta_path):
                    for file in files:
                        ...
                        if (await get_document_type(video_file))[0]:
                            outfile = ospath.join(self.newDir, file)
                            await cut_video_segments(self, dirpath, video_file, outfile, segments)

        # --- METADATA, INTROSUB & ATTACHMENT (Combined Logic) ---
        general_meta = self.user_dict.get('general_metadata') or config_dict['GENERAL_METADATA']
        video_meta = self.user_dict.get('video_metadata') or config_dict['VIDEO_METADATA']
        audio_meta = self.user_dict.get('audio_metadata') or config_dict['AUDIO_METADATA']
        subtitle_meta = self.user_dict.get('subtitle_metadata') or config_dict['SUBTITLE_METADATA']
        introsub = self.user_dict.get('introsub') or config_dict['INTROSUB']
        attachment = self.user_dict.get('attachment') or config_dict['ATTACHMENT']
        attachment_name = self.user_dict.get('attachment_name') or config_dict['ATTACHMENT_NAME']
        auto_attachment = self.user_dict.get('auto_attachment', False)

        # Check if ANY edit is required (Updated to include video_meta and attachment_name)
        if general_meta or video_meta or audio_meta or subtitle_meta or introsub or attachment or attachment_name or auto_attachment:
            meta_path = up_path
            self.newDir = f'{self.dir}10000'
            await makedirs(self.newDir, exist_ok=True)
            ...
            if await aiopath.isfile(meta_path) and (await get_document_type(meta_path))[0]:
                ...
                if not current_attachment and auto_attachment:
                    # Lookup using the filename
                    if poster := await get_tmdb_lookup(file_name):
                        current_attachment = poster
                        LOGGER.info(f"Auto-attached Poster: {poster}")
                # -------------------------

                await edit_metadata(self, base_dir, meta_path, outfile,
                ...
                for dirpath, _, files in await sync_to_async(walk, meta_path):
                    for file in files:
                        ...
                        if (await get_document_type(video_file))[0]:
                            ...
                            # --- AUTO ATTACH LOGIC (Per File) ---
                            current_attachment = attachment
                            if not current_attachment and auto_attachment:
                                # Lookup using the specific file's name
                                if poster := await get_tmdb_lookup(file):
                                    current_attachment = poster
                            # ------------------------------------

                            await edit_metadata(self, dirpath, video_file, outfile,
                                                general_metadata=general_meta,
                                                video_metadata=video_meta,
                                                audio_metadata=audio_meta,
                                                subtitle_metadata=subtitle_meta,
        ...
        if self.isLeech:
            ...
            current_settings = user_data.get(self.user_id, {})
            dump_chat = current_settings.get('ldump', {})
            dump_all = current_settings.get('dump_all', False)
            dump_selection = current_settings.get('dump_selection', [])
            
            original_dumps = None

            if dump_all and isinstance(dump_chat, dict) and len(dump_chat) > 1:
                if dump_selection:
                    target_ids = [id_ for name, id_ in dump_chat.items() if name in dump_selection]
                else:
                    target_ids = list(dump_chat.values())

                if target_ids:
                    LOGGER.info(f"Dump All: Uploading to {len(target_ids)} selected dumps.")
                    original_dumps = dump_chat.copy()
                    user_data[self.user_id]['ldump'] = {'Batch Upload': target_ids}
                    self.dump_chat = target_ids
                    self.upDest = target_ids
                    self.upPath = target_ids 
            ...
            try:
                ...
                if original_dumps:
                    user_data[self.user_id]['ldump'] = original_dumps
...
┖ <b>Elapsed:</b> {get_readable_time(time() - self.message.date.timestamp())}'''
        ...
        # FIX: Check if bot is restarting via config_dict before removing from DB
        if self.isSuperGroup and config_dict['INCOMPLETE_TASK_NOTIFIER'] and DATABASE_URL:
             if not config_dict.get('IS_RESTARTING'):
                await DbManger().rm_complete_task(self.message.link)
