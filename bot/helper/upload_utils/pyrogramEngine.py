...
class TgUploader:
    ...
    async def __user_settings(self):
        ...
        # --- FIX FOR DUMP ALL ---
        # Check dump_chat first (set by tasks_listener), fallback to upPath
        ud = getattr(self.__listener, 'dump_chat', None) or self.__listener.upPath
        # ------------------------
        
        self.__upload_dest = ud if isinstance(ud, list) else [ud] if ud else []
        self.__has_buttons = bool(config_dict['SAVE_MSG'] or self.__mediainfo or self.__leech_utils['screenshots'])
        # ✅ Validate final thumbnail file path
    ...
    async def __prepare_file(self, prefile_, dirpath):
        ...
        thumb_path = f"Thumbnails/{self.__user_id}.jpg"
        if await aiopath.exists(thumb_path):
            self.__thumb = thumb_path
